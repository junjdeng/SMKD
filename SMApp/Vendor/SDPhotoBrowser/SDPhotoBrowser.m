...
{
    // 1. 序标
    if (self.subscriptType == SubscriptTypeUpNumber||self.subscriptType == SubscriptTypeDownNumber) {//数字
    ...
    }else if (self.subscriptType == SubscriptTypeUpPoint||self.subscriptType == SubscriptTypeDownPoint){
        UIPageControl *pageControl = [[UIPageControl alloc] init];
        pageControl.bounds = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width-180, 30);
...
- (void)layoutSubviews
{
    ...
    if (self.subscriptType == SubscriptTypeUpNumber) {//上数字
        _indexLabel.center = CGPointMake(self.bounds.size.width * 0.5, 70-15);//加label高度的一半
    }else if (self.subscriptType == SubscriptTypeDownNumber) {//下数字
        _indexLabel.center = CGPointMake(self.bounds.size.width * 0.5, [UIScreen mainScreen].bounds.size.height-70+15);//
    }else if (self.subscriptType == SubscriptTypeUpPoint) {//上点
        _pageControl.center = CGPointMake(self.bounds.size.width * 0.5, 70-15);//
    }else if (self.subscriptType == SubscriptTypeDownPoint) {//下点
        _pageControl.center = CGPointMake(self.bounds.size.width * 0.5, [UIScreen mainScreen].bounds.size.height-70+15);//
    }
    _saveButton.frame = CGRectMake(30, self.bounds.size.height - 70, 50, 30);
    if (_isHaveSaveBtn) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (!_willDisappear) {
        if (self.subscriptType == SubscriptTypeUpPoint||self.subscriptType == SubscriptTypeDownPoint) {
            _pageControl.currentPage = index;
        }else if (self.subscriptType == SubscriptTypeUpNumber||self.subscriptType == SubscriptTypeDownNumber) {
