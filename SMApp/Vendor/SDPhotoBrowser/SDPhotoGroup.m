...
- (void)setPhotoItemArray:(NSArray *)photoItemArray
{
    ...
    [photoItemArray enumerateObjectsUsingBlock:^(SDPhotoItem *obj, NSUInteger idx, BOOL *stop) {
        ...
        [btn sd_setImageWithURL:[NSURL URLWithString:obj.thumbnail_pic] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"default_220x220"]];
        btn.tag = idx;
    ...
    NSInteger totalRowCount = ceil(imageCount / perRowImageCountF);
//    CGFloat h = 80;
//    self.frame = CGRectMake(SDPhotoGroupImageMargin, SDPhotoGroupImageMargin, kScreenWidth-2*SDPhotoGroupImageMargin, totalRowCount * (SDPhotoGroupImageMargin + h));
    
    CGFloat w = ([UIScreen mainScreen].bounds.size.width-4*SDPhotoGroupImageMargin)/3;
    CGFloat h = w;
    self.groupViewHeight = totalRowCount*(SDPhotoGroupImageMargin + h);
}
...
- (void)layoutSubviews
{
    ...
    [self.subviews enumerateObjectsUsingBlock:^(UIButton *btn, NSUInteger idx, BOOL *stop) {
        ...
        CGFloat x = columnIndex * (w + SDPhotoGroupImageMargin)+SDPhotoGroupImageMargin;
...
- (void)buttonClick:(UIButton *)button
{
    ...
    browser.subscriptType = self.subscriptType;
    browser.isHaveSaveBtn = self.isHaveSaveBtn;
