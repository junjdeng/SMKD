...
- (void)setURLString:(NSString *)URLString{
    ...
    //AVPlayerLayer
    self.playerLayer = [AVPlayerLayer playerLayerWithPlayer:self.player];
    self.playerLayer.frame = CGRectMake(0, 0, 200, 40);
    //WMPlayer视频的默认填充模式，AVLayerVideoGravityResizeAspect
//    self.playerLayer.videoGravity = AVLayerVideoGravityResize;
    [self.layer insertSublayer:_playerLayer atIndex:0];
    self.state = WMPlayerStateBuffering;
    if (self.closeBtnStyle==CloseBtnStylePop) { // WMPlayerSrcName(play_back.png)
        [_closeBtn setImage:[UIImage imageNamed:@"back"] ?: [UIImage imageNamed:@"back"] forState:UIControlStateNormal];
        [_closeBtn setImage:[UIImage imageNamed:@"back"] ?: [UIImage imageNamed:@"back"] forState:UIControlStateSelected];
